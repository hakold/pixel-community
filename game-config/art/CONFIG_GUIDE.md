...
| 图片类型 | 用途 | 必须尺寸（宽×高） | 举例 |
|---------|------|------------------|------|
| 地块 | 草地、道路、地板 | **96 × 60 像素** | grass_01.png |
| 建筑 | 矿场、医院、学校等 | **96 × 110 像素** | mine.png |
...
例如：地图上所有建筑现在显示"矿场"，我想换成"医院"。

1. 同样的文件 `game-config/art/tile-manifest.json`
2. 找到 `"building"` 这一段
3. 把 `sprite` 路径改成 `/assets/sprites/buildings/hospital.png`
4. 保存，刷新
...
| 我要做的事 | 改哪个文件 | 在哪 |
|-----------|-----------|------|
| 换地块/建筑的图 | `tile-manifest.json` | `game-config/art/` |
...
**Q：为什么所有建筑都显示同一张图？**
A：目前所有建筑在地图数据中都是同一种类型 `building`。要不同建筑显示不同图，需要给地图增加建筑子类型。这是后续功能。

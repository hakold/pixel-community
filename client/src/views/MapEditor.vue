      ...
      <section class="canvas-section">
        <div class="toolbar">
          ...
          <button v-for="t in tileTypes" :key="t.code"
            class="brush-btn"
            :class="{ active: !pickMode && brush === t.code }"
            :style="{ borderColor: !pickMode && brush === t.code ? t.color : 'transparent' }"
...
const tileTypes = [
  { code: 'g', label: '草地', color: '#97c977' },
  { code: 'r', label: '道路', color: '#e6d8bc' },
  { code: 'i', label: '室内', color: '#f2e5cc' },
  { code: 'b', label: '建筑', color: '#b8795a' },
  { code: 'd', label: '门口', color: '#d8c4a0' }
];
...
const mapConfig = reactive({
  ...
  legend: { g: 'grass', r: 'road', i: 'interior', b: 'building', d: 'door' },
  portals: [], dummies: [], actionZones: [], tiles: []
});
...
function renderGrid() {
  ...
  for (const d of mapConfig.dummies) {
    const cx = d.spawnX * CELL + CELL / 2, cy = d.spawnY * CELL + CELL / 2;
    ctx.fillStyle = d.bodyColor || '#6ea4bf';
    ctx.beginPath(); ctx.arc(cx, cy, 5, 0, Math.PI * 2); ctx.fill();
  }

  // 出生点
  const sx = mapConfig.player.spawnX * CELL + CELL / 2, sy = mapConfig.player.spawnY * CELL + CELL / 2;
...
function resizeGrid() {
  ...
  for (let y = 0; y < h; y++) {
    while (mapConfig.tiles[y].length < w) mapConfig.tiles[y].push('g');
...
function onMouseDown(e) {
  ...
  if (cell) { mapConfig.tiles[cell.y][cell.x] = brush.value; dirty.value = true; renderGrid(); }
}
...
function normalizeMap(raw) {
  ...
  raw.legend ??= { g: 'grass', r: 'road', i: 'interior', b: 'building', d: 'door' };
  raw.portals ??= []; raw.dummies ??= []; raw.actionZones ??= [];
  raw.tiles ??= Array.from({ length: raw.grid.height }, () => Array(raw.grid.width).fill('g'));
  resizeTiles(raw);
  return raw;
}
...
function resizeTiles(cfg) {
  ...
  for (let y = 0; y < cfg.grid.height; y++) {
    while (cfg.tiles[y].length < cfg.grid.width) cfg.tiles[y].push('g');
